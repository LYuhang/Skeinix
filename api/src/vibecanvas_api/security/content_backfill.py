...
async def backfill_agent_plan(session: AsyncSession, plan_id: str) -> int:
    """Encrypt one Agent Plan and its phase replay ledger."""
    plan = (
        await session.execute(
            text(
                "SELECT plan_id, run_id, tenant_id, chat_id, title, phases, "
                "private_key_id FROM agent_plans WHERE plan_id=:plan_id FOR UPDATE"
            ),
            {"plan_id": plan_id},
        )
    ).mappings().one_or_none()
    ...
    crypto = content_encryption_service()
    migrated = 0
    if plan["private_key_id"] is None:
        encrypted = await crypto.encrypt_json(
            session,
            tenant_id=plan["tenant_id"],
            resource_type="chat",
            resource_id=plan["chat_id"],
            purpose="agent_plan_private",
            record_id=plan_id,
            value={
                "title": plan["title"],
        ...
        await session.execute(
            text(
                "UPDATE agent_plans SET title='', phases='[]'::jsonb, "
                "private_ciphertext=:ciphertext, private_nonce=:nonce, "
                "private_key_id=:key_id WHERE plan_id=:plan_id"
            ),
            {
                "ciphertext": encrypted.ciphertext,
                "nonce": encrypted.nonce,
    ...
    events = list(
        (
            await session.execute(
                text(
                    "SELECT id, payload FROM phase_events WHERE run_id=:run_id "
                    "AND payload_key_id IS NULL ORDER BY id FOR UPDATE"
                ),
                {"run_id": plan["run_id"]},
            )
        ).mappings()
    ...
    for event in events:
        ...
        await session.execute(
            text(
                "UPDATE phase_events SET payload='{}'::jsonb, "
                "payload_ciphertext=:ciphertext, payload_nonce=:nonce, "
                "payload_key_id=:key_id WHERE id=:event_id"
            ),
            {
