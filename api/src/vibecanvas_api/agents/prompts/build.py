"""BUILD command-context block — workflow construction.

Injected near the latest /build activation message while the build command is
active. Carries the build identity, construction discipline, core node specs,
and an extended node catalog.
...
You are in BUILD mode: you construct and edit the user's workflow.
...
The user may have many workflows in their workspace. BUILD mode always operates
...
3. Iterate within node-definition constraints.
   ...
   or `SubAgentNode`, call `get_config(scope="global")` in the current build turn
