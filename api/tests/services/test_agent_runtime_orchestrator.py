...
from vibecanvas_api.services.agent_runtime.orchestrator import (
    ...
    _product_events,
    _runtime_events_with_diagram_continuations,
...
def _diagram_turn_request(**updates) -> RuntimeTurnRequest:
    values = {
        "tenant_id": "tenant",
        "user_id": "user",
        "chat_id": "chat",
        ...
        "attachments": [{"type": "image", "path": "/data/input.png"}],
        "active_platform_mcps": ["diagram"],
        "mcp_servers": [{
            "name": "diagram",
            "source": "platform",
            "connection": {
                "transport": "streamable_http",
                "url": "http://localhost/api/internal/mcp/diagram/",
            },
        }],
        "instructions": [{
            "instruction_id": "command.diagram.v1",
            "kind": "command_context",
            "scope": "chat",
            "name": "diagram",
            "version": 1,
            "content": "Diagram protocol",
            "activated_this_turn": True,
        }],
        "command_context": {
            "active_modes": ["diagram"],
            "activated_this_turn": ["diagram"],
        },
    }
...
@pytest.mark.asyncio
async def test_diagram_protocol_continues_on_same_runtime_state_until_deliver():
    first = [
        _runtime_event("checkpoint", seq=1, payload={"state_ref": "thread-1"}),
        _runtime_event(
            "tool.end",
            seq=2,
            payload={
                "name": "mcp__diagram__review_diagram",
                "status": "done",
                "content": '{"next":{"action":"edit_source"}}',
            },
        ),
        _runtime_event("runtime.completed", seq=3),
    ]
    second = [
        _runtime_event(
            "tool.end",
            ...
            payload={
                ...
                "status": "done",
                "artifact": {
                    "structured_content": {"next": {"action": "deliver"}}
                },
            },
        ),
        _runtime_event("runtime.completed", seq=5),
    ]
    runtime = _ScriptedRuntime([first, second])

    events = [
        event
        async for event in _runtime_events_with_diagram_continuations(
            runtime, _diagram_turn_request()
        )
    ]

    assert len(runtime.requests) == 2
    continuation = runtime.requests[1]
    assert continuation.runtime_state_ref == "thread-1"
    assert continuation.continuation_index == 1
    assert continuation.attachments == []
    assert "not a new user request" in continuation.message["content"]
    assert [event.type for event in events].count("runtime.completed") == 1


@pytest.mark.asyncio
async def test_diagram_protocol_preserves_required_render_after_later_review_tools():
    first = [
        _runtime_event(
            "tool.end",
            seq=1,
            payload={
                "status": "done",
                "content": "Diagram is ready",
                "invocation": {
                    "name": "mcp__diagram__check_diagram",
                    "output": {
                        "structured_content": {
                            "next": {
                                "action": "call_tool",
        ...
        _runtime_event(
            ...
            payload={
                "name": "review_diagram",
                "status": "done",
                "content": '{"next":{"action":"deliver"}}',
            },
        ),
        _runtime_event(
            "tool.end",
            seq=3,
            payload={"name": "read_images", "status": "done", "content": "ok"},
        ),
        _runtime_event("runtime.completed", seq=4),
    ]
    second = [
        _runtime_event(
            "tool.end",
        ...
        _runtime_event("runtime.completed", seq=6),
    ]
    runtime = _ScriptedRuntime([first, second])
    ...
    assert len(runtime.requests) == 2
    assert "render_interactive" in runtime.requests[1].message["content"]
    assert "/data/diagrams/flow.vdiagram.json" in runtime.requests[1].message["content"]
    assert [event.type for event in events].count("runtime.completed") == 1


@pytest.mark.asyncio
async def test_diagram_protocol_retries_latest_failed_tool_but_stops_for_hitl():
    failed = [
        _runtime_event(
            "tool.end",
            seq=1,
            payload={
                ...
                "status": "failed",
                "content": "The exact presented revision changed.",
            },
        ),
        _runtime_event("runtime.completed", seq=2),
    ]
    gated = [
        _runtime_event(
            "tool.end",
        ...
        _runtime_event("interaction.required", seq=4),
        _runtime_event("runtime.completed", seq=5),
    ]
    runtime = _ScriptedRuntime([failed, gated])
    ...
    assert len(runtime.requests) == 2
    assert "failed" in runtime.requests[1].message["content"]
    assert [event.type for event in events].count("runtime.completed") == 1


@pytest.mark.asyncio
async def test_diagram_protocol_continuation_is_bounded_to_three():
    retry = [
        _runtime_event(
            "tool.end",
            seq=1,
            payload={
                "name": "check_diagram",
                "status": "done",
                "content": '{"next":{"action":"call_tool"}}',
            },
        ),
        _runtime_event("runtime.completed", seq=2),
    ]
    runtime = _ScriptedRuntime([retry])
    ...
    assert [request.continuation_index for request in runtime.requests] == [0, 1, 2, 3]
    assert [event.type for event in events].count("runtime.completed") == 1
