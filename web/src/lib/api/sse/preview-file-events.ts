      ...
      onmessage(message) {
        ...
        try {
          ...
          if (
            ...
            if (
              fileRef.path.endsWith('.vdiagram.json')
              && typeof payload.committed_at === 'string'
            ) {
              const committedTimestamp = Date.parse(payload.committed_at);
              const committedEventId = Number(payload.committed_event_id);
              const timelineEventId = Number.isFinite(committedEventId)
                ? committedEventId
                : eventId;
              if (Number.isFinite(committedTimestamp)) {
            ...
            if (
              ...
              const committedTimestamp = Date.parse(committedAt);
              if (Number.isFinite(committedTimestamp)) {
                recordDiagramPreviewTimeline({
                  stage: 'T0',
                  path: fileRef.path,
