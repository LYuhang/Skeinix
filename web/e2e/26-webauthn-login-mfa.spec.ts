...
test('real WebAuthn enrollment protects the next password login', async ({
  context,
  page,
}) => {
  ...
  try {
    ...
    await expect(page.getByTestId('mfa-security-card')).toBeVisible();
    ...
    expect(credentials.credentials[0]?.signCount).toBeGreaterThanOrEqual(1);
  } finally {
